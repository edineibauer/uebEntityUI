...
foreach (\Helpers\Helper::listFolder("entity/cache") as $json) {
    if($json !== "info" && !in_array($json, ["api_chave.json", "config.json", "dashboard_note.json", "dashboard_push.json", "email_envio.json", "login_attempt.json", "usuarios_token.json"]) && preg_match('/\.json$/i', $json)) {
