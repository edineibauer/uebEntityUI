...
function saveAttrValue($input) {
    ...
    if (["default", "size"].indexOf(name) > -1 && !$("#" + name + "_custom").prop("checked"))
        ...
        return;
    else if (["class", "style", "orientation", "template", "atributos", "coll", "cols", "colm", "input", "type"].indexOf(name) > -1)
...
function allowName(nome, tipo) {
    ...
    if (typeof nome !== "undefined") {
        ...
        if (tipo === 2 && nome.length > 2 && (entity.edit < 1 || (entity.edit > 0 && nome !== dicionariosEdit[entity.name][entity.edit].nome.toLowerCase().trim()))) {
            let tt = slug(nome, "_");
            nameColumnTest = !0;
            $.each(dicionariosEdit[entity.name], function (i, e) {
